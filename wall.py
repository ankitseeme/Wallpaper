...
drive_loc = ""
copy_from_loc = ""
#drive_loc = "/media/ankit/New Volume/Wallpapers"
extensions = ['.jpg','.png']
...
def find_duplicates(drive_loc):
	...
	if count == 0:
		...
		print("Advised to run the script with -r to rename the files after deleting the duplicates")
...
if __name__ == '__main__':
	...
	if len(sys.argv) == 1:
		...
		for arg in args:
			if arg not in valid_args:
				print("invalid argument: " + arg + " in " +  ''.join(sys.argv[1]))
				usage()
				exit(1)
		if 'r' in args:
		...
		if 'c' in args and 'r' in args:
			...
			if drive_loc == copy_from_loc:
				...
				exit(1)
			copy_wallpapers(copy_from_loc,drive_loc,count)
			print()
	...
	"""elif sys.argv[1] == '-r':
		drive_loc = input("Provide the wallpaper directory path: ")
		if not os.path.isdir(drive_loc):
			print("Not a valid directory")
		else:
			latest = find_rename(drive_loc)
	elif sys.argv[1] == '-d':
		drive_loc = input("Provide the wallpaper directory path: ")
		if not os.path.isdir(drive_loc):
			print("Not a valid directory")
		...
		copy_from_loc = input("Provide the directory to copy from: ")
		if not os.path.isdir(copy_from_loc):
			print("Not a valid source directory")
			exit(1)
		drive_loc = input("Provide the wallpaper directory path: ")
		if not os.path.isdir(drive_loc):
			print("Not a valid target directory")
			exit(1)
		else:
			new_count = find_rename(drive_loc)
			copy_wallpapers(copy_from_loc,drive_loc,new_count)"""
